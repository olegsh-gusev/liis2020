...
def reservation_info(request):
    ...
    return JsonResponse(
        {'Answer': ReservationSerializer(Reservation.objects.filter(workplace=reservation_id), many=True).data},
        status=HTTP_200_OK)
...
def reservation(request):
    try:
        reservation_id = int(request.data.get('id'))
    except (TypeError, ValueError):
        return response_incorrect_id()
    try:
    ...
    if not workplace.exists():
        return JsonResponse({'Error': 'WorkPlace with this id does not exists'}, status=HTTP_400_BAD_REQUEST)
    if workplace.exclude(reservation__in=get_reservation(datetime_from, datetime_to)).exists():
        Reservation.objects.create(user=request.user, workplace=workplace.first(), date_from=datetime_from,
                                   date_to=datetime_to)
        return JsonResponse({'Answer': 'Reservation created'}, status=HTTP_200_OK)
    else:
        return JsonResponse({'Error': 'Cannot make reservation for this period of time'}, status=HTTP_400_BAD_REQUEST)
...
def register(request):
    ...
    if first_name == '' and last_name == '':
        return JsonResponse({'Error': 'add first_name or last_name'}, status=HTTP_400_BAD_REQUEST)
    if email is not None:
        try:
            validate_email(email)
        except ValidationError:
            return JsonResponse({'Error': 'Not valid email'}, status=HTTP_400_BAD_REQUEST)
    ...
    User.objects.create_user(username=username, first_name=first_name, last_name=last_name, email=email,
                             password=password)
    return JsonResponse({'Username': username}, status=HTTP_200_OK)
...
def vacant_space(request):
    try:
        datetime_from, datetime_to = get_date(request, True)
    except (ParserError, ValueError) as e:
        return response_incorrect_date(e)
    return JsonResponse({'Answer': WorkPlaceSerializer(
        WorkPlace.objects.exclude(reservation__in=get_reservation(datetime_from, datetime_to)),
        many=True).data},
                        status=HTTP_200_OK)
